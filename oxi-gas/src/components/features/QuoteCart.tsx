...
type Product = { id: string; code: string; name: string; brand?: string; category?: string; images?: string[]; custom_fields?: CustomField[] };

type QuoteCartProps = { cart: QuoteCartType };
...
export function QuoteCart({ cart }: QuoteCartProps) {
  ...
  useEffect(() => {
    supabase.from('products').select('id, code, name, brand, category, images, custom_fields')
      .then(({ data }) => { if (data) setAllProducts(data as Product[]); });
  }, []);
  ...
  return (
    <>
      <AnimatePresence>
        {cart.count > 0 && (
          <motion.button type="button" onClick={() => setOpen(true)}
            ...
            exit={{ opacity: 0, scale: 0.6, y: 20 }} transition={{ duration: 0.25, ease: 'easeOut' }}
            ...
            <span className="relative inline-flex">
              ...
              <span className="absolute -top-2 -right-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1 rounded-full bg-white text-primary text-xs font-bold">{cart.count}</span>
            </span>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md flex flex-col bg-[hsl(var(--surface-1))] border-l border-[hsl(var(--surface-3))] text-[hsl(var(--text-main))]">
          <SheetHeader className="text-left">
            <SheetTitle className="flex items-center gap-2 text-[hsl(var(--text-main))]">
              <ShoppingCart className="h-5 w-5 text-primary" />Mi cotización
            </SheetTitle>
          ...
          <div className="flex-1 overflow-y-auto -mx-6 px-6 py-4">
            {enrichedItems.length === 0 ? <EmptyCart /> : (
              <ul className="flex flex-col gap-3">
                {enrichedItems.map(({ product, cartItem }) => (
                  <CartItemRow key={product.code} product={product} cartItem={cartItem} onRemove={() => cart.remove(product.code)} />
                ))}
              </ul>
            )}
          </div>
          ...
          {enrichedItems.length > 0 && (
            <SheetFooter className="flex-col gap-3 sm:flex-col sm:space-x-0 border-t border-[hsl(var(--surface-3))] pt-4">
              <a href={buildWhatsAppHref(enrichedItems)} target="_blank" rel="noopener noreferrer"
                ...
                Cotizar {enrichedItems.length} {enrichedItems.length === 1 ? 'producto' : 'productos'} por WhatsApp
              </a>
...
function CartItemRow({ product, cartItem, onRemove }: { product: Product; cartItem: CartItem; onRemove: () => void }) {
  ...
  return (
    <li className="flex items-start gap-3 rounded-xl border border-[hsl(var(--surface-3))] bg-[hsl(var(--surface-2))] p-3">
      <div className="flex-1 min-w-0">
        <p className="text-xs font-mono uppercase tracking-wider text-[hsl(var(--text-soft))]">{product.code}</p>
        <p className="mt-0.5 text-sm font-semibold text-[hsl(var(--text-main))] leading-snug">{product.name}</p>
        <p className="mt-1 text-xs text-[hsl(var(--text-soft))]">{product.brand}</p>
        {fieldSummary && (
          <p className="mt-1.5 text-xs font-medium text-amber-600 bg-amber-500/10 rounded px-2 py-1 leading-snug">{fieldSummary}</p>
        )}
      </div>
      <button type="button" onClick={onRemove} aria-label={`Quitar ${product.name} de la cotización`}
...
function EmptyCart() {
  return (
    <div className="flex flex-col items-center justify-center text-center py-12">
      ...
      <p className="mt-1 text-sm text-[hsl(var(--text-soft))] max-w-xs">Tocá el casillero de cada producto que quieras incluir en tu cotización.</p>
    </div>
  );
}